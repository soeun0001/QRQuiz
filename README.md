...
GitHub에 올릴 파일은 이 폴더 전체입니다. 파일명은 GitHub Pages 호환을 위해 모두 영문으로 정리되어 있습니다.
...
- `quiz-season3-home.png`: 시작 화면 배경 이미지
- `assets/nature-logo.svg`: 보조 로고 이미지
- `assets/snake-hint.svg`: 예시 힌트 이미지

GitHub Pages에서 `main` 브랜치의 `/root`로 바로 배포할 수 있도록 모든 핵심 파일이 저장소 최상위에 있습니다.
...
관리자 페이지의 문제 편집 영역에서 문제마다 사진, 영상, 오디오를 추가할 수 있습니다.

- 사진: `image/*`
- 영상: `mp4`
- 오디오: `mp3`

파일을 업로드하면 관리자 페이지가 Data URL로 변환해 `questions.json`에 저장합니다. 그래서 GitHub Pages에서도 별도 서버 없이 바로 재생됩니다.

영상 파일은 용량이 크면 `questions.json`도 커집니다. 현장 운영용으로는 짧고 가벼운 mp4 파일을 사용하는 것을 권장합니다.

미디어가 없는 문제는 기존처럼 텍스트와 선택지만 표시됩니다.
